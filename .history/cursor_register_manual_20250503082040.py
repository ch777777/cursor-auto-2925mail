...
class CursorRegistration:
    ...
    def get_verification_code(self):
        ...
        try:
            ...
            # 先登录2925邮箱
            print(f"{Fore.CYAN}{EMOJI['INFO']} 正在登录2925邮箱...{Style.RESET_ALL}")
            verification_page.get("https://www.2925.com/login/")
            time.sleep(3)  # 等待页面加载
            
            # 输入账号
            account_input = verification_page.ele("@name=accountName")
            if account_input:
                account_input.input("chanan72")
                time.sleep(1)
            
            # 输入密码
            password_input = verification_page.ele("@name=pwd")
            if password_input:
                password_input.input("mzxgfwlpt00")
                time.sleep(1)
            
            # 勾选用户协议
            checkbox = verification_page.ele("css:.iCheck-helper")
            if checkbox:
                checkbox.click()
                time.sleep(1)
            
            # 点击登录按钮
            login_button = verification_page.ele("@id=btn_login")
            if login_button:
                login_button.click()
                time.sleep(3)  # 等待登录完成
            ...
            for attempt in range(max_attempts):
                ...
                for i, (time_ele, title_ele) in enumerate(zip(recent_emails, verify_emails)):
                    if time_ele.text == "刚刚" and "Verify your email address" in title_ele.text:
                        title_ele.click()
                        time.sleep(3)  # 等待邮件内容加载
