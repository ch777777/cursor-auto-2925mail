                        ...
                        if "Verify your email address" in email.text:
                            ...
                            if code_element:
                                ...
                                if code.isdigit() and len(code) == 6:
                                    ...
                                    # 勾选所有邮件
                                    print(f"{Fore.CYAN}{EMOJI['INFO']} 正在勾选邮件...{Style.RESET_ALL}")
                                    # 使用set记录已点击的复选框
                                    clicked_checkboxes = set()
                                    checkboxes = verification_page.eles("css:.ivu-checkbox-inner")
                                    for checkbox in checkboxes:
                                        try:
                                            # 获取复选框的唯一标识（如位置信息）
                                            checkbox_id = str(checkbox.rect)
                                            if checkbox_id not in clicked_checkboxes:
                                                checkbox.click()
                                                clicked_checkboxes.add(checkbox_id)
                                                time.sleep(0.5)
                                        except:
                                            pass
                                    
                                    # 点击删除按钮
                                    print(f"{Fore.CYAN}{EMOJI['INFO']} 正在删除邮件...{Style.RESET_ALL}")
                                    # 先尝试通过文本内容查找删除按钮
                                    delete_buttons = verification_page.eles("css:span[data-v-5f3c3253]")
