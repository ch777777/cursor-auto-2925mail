...
def setup_email(translator=None):
    ...
    try:
        ...
        # 点击立即注册按钮
        submit_button = page.ele("css:.register-btn.register-btn-allowed")
        if submit_button:
            print(f"{Fore.CYAN}{EMOJI['INFO']} 找到注册按钮，正在点击...{Style.RESET_ALL}")
            submit_button.click()
            print(f"{Fore.CYAN}{EMOJI['WAIT']} 等待注册完成...{Style.RESET_ALL}")
            ...
            for _ in range(max_wait):
                ...
                if "login" in current_url:  # 如果URL包含login，说明注册成功并跳转到登录页
                    success = True
                    break
                time.sleep(1)
            
            if not success:
                print(f"{Fore.RED}{EMOJI['ERROR']} 注册超时，请检查是否注册成功{Style.RESET_ALL}")
                input(f"{Fore.CYAN}请确认是否继续(按回车继续)...{Style.RESET_ALL}")
        else:
            print(f"{Fore.RED}{EMOJI['ERROR']} 未找到注册按钮，尝试备用方案...{Style.RESET_ALL}")
            # 备用方案：通过span文本查找
            submit_button = page.ele("xpath://div[contains(@class, 'register-btn')]//span[text()='立即注册']")
            if submit_button:
                print(f"{Fore.CYAN}{EMOJI['INFO']} 通过备用方案找到注册按钮，正在点击...{Style.RESET_ALL}")
                submit_button.click()
                print(f"{Fore.CYAN}{EMOJI['WAIT']} 等待注册完成...{Style.RESET_ALL}")
                ...
                print(f"{Fore.RED}{EMOJI['ERROR']} 未找到注册按钮，请手动点击注册{Style.RESET_ALL}")
