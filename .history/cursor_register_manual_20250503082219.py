...
class CursorRegistration:
    ...
    def get_verification_code(self):
        ...
        try:
            ...
            # 访问验证页面
            print(f"{Fore.CYAN}{EMOJI['INFO']} 正在访问邮箱页面...{Style.RESET_ALL}")
            verification_page.get("https://www.2925.com/#/mailList")
            time.sleep(5)  # 等待页面加载
            
            # 输入邮箱地址
            email_input = verification_page.ele("css:input[type='text']")
            if email_input:
            ...
            for attempt in range(max_attempts):
                ...
                print(f"{Fore.YELLOW}{EMOJI['WAIT']} 等待验证码出现，尝试次数: {attempt + 1}/{max_attempts}{Style.RESET_ALL}")
                time.sleep(3)
