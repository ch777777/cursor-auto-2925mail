    ...
    def get_verification_code(self):
        ...
        try:
            ...
            for attempt in range(max_attempts):
                ...
                try:
                    # 查找所有邮件项
                    email_items = verification_page.eles("css:.mail-item")
                    for item in email_items:
                        # 在每个邮件项中查找时间标签
                        time_text = item.ele("css:.date-time-text")
                        if time_text and "刚刚" in time_text.text:
                            # 在同一个邮件项中查找标题
                            title = item.ele("css:.mail-content-title")
