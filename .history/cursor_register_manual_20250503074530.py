...
class CursorRegistration:
    ...
    def setup_email(self):
        """Setup Email"""
        try:
            # Generate random 1-2 digit number (excluding 1-6)
            allowed_digits = list(range(7, 100))  # 7-99
            random_number = str(random.choice(allowed_digits))
    ...
    def get_verification_code(self):
        ...
        try:
            ...
            verify_email = verification_page.ele("@class=mail-content-title")
            if verify_email and "Verify your email address" in verify_email.text:
                verify_email.click()
                ...
                if code_element:
                    ...
                    if code.isdigit() and len(code) == 6:
                        print(f"{Fore.GREEN}{EMOJI['SUCCESS']} {self.translator.get('register.verification_code_found') if self.translator else 'Verification code found'}: {code}{Style.RESET_ALL}")
                        verification_page.quit()
                        return code
            
            verification_page.quit()
            print(f"{Fore.RED}{EMOJI['ERROR']} {self.translator.get('register.verification_code_not_found') if self.translator else 'Verification code not found'}{Style.RESET_ALL}")
            return None
