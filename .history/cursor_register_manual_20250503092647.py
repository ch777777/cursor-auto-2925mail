...
def custom_2925_email(translator=None):
    """Setup custom email account on 2925.com"""
    try:
        # 获取用户输入
        print(f"{Fore.CYAN}{EMOJI['INFO']} 请输入注册信息：{Style.RESET_ALL}")
        while True:
            ...
            if len(email_name) < 9 or len(email_name) > 12:
                print(f"{Fore.RED}{EMOJI['ERROR']} 邮箱名长度必须在9-12个字符之间{Style.RESET_ALL}")
                continue
            if not any(c.isalpha() for c in email_name):
                print(f"{Fore.RED}{EMOJI['ERROR']} 邮箱名必须包含字母{Style.RESET_ALL}")
                continue
        ...
        while True:
            ...
            if len(password) < 6 or len(password) > 16:
                print(f"{Fore.RED}{EMOJI['ERROR']} 密码长度必须在6-16个字符之间{Style.RESET_ALL}")
                continue
            break
