...
def setup_email(translator=None):
    ...
    try:
        ...
        if email_input:
            ...
            time.sleep(1)
            # 检查邮箱输入提示
            error_text = page.ele('xpath://input[@id="reg-account"]/../../div[contains(@class, "el-form-item__error")]')
            if error_text and error_text.text.strip():
                print(f"{Fore.RED}{EMOJI['ERROR']} 邮箱错误: {error_text.text.strip()}{Style.RESET_ALL}")
                return False
        ...
        if password_input:
            ...
            time.sleep(1)
            # 检查密码输入提示
            error_text = page.ele('xpath://input[@id="reg-pwd"]/../../div[contains(@class, "el-form-item__error")]')
            if error_text and error_text.text.strip():
                print(f"{Fore.RED}{EMOJI['ERROR']} 密码错误: {error_text.text.strip()}{Style.RESET_ALL}")
                return False
        ...
        if confirm_password_input:
            ...
            time.sleep(1)
            # 检查确认密码提示
            error_text = page.ele('xpath://input[@id="reg-confirm"]/../../div[contains(@class, "el-form-item__error")]')
            if error_text and error_text.text.strip():
                print(f"{Fore.RED}{EMOJI['ERROR']} 确认密码错误: {error_text.text.strip()}{Style.RESET_ALL}")
                return False
        ...
        if phone_input:
            ...
            time.sleep(1)
            # 检查手机号码提示
            error_text = page.ele('xpath://input[@id="reg-phone"]/../../div[contains(@class, "el-form-item__error")]')
            if error_text and error_text.text.strip():
                print(f"{Fore.RED}{EMOJI['ERROR']} 手机号码错误: {error_text.text.strip()}{Style.RESET_ALL}")
                return False
        ...
        if code_button:
            ...
            time.sleep(1)
            # 检查验证码发送提示
            error_text = page.ele('xpath://button[@id="codeBtn"]/../../div[contains(@class, "el-form-item__error")]')
            if error_text and error_text.text.strip():
                print(f"{Fore.RED}{EMOJI['ERROR']} 验证码错误: {error_text.text.strip()}{Style.RESET_ALL}")
                return False
        ...
        if code_input:
            ...
            time.sleep(1)
            # 检查验证码输入提示
            error_text = page.ele('xpath://input[@id="reg-message"]/../../div[contains(@class, "el-form-item__error")]')
            if error_text and error_text.text.strip():
                print(f"{Fore.RED}{EMOJI['ERROR']} 验证码错误: {error_text.text.strip()}{Style.RESET_ALL}")
                return False
        ...
        if submit_button:
            submit_button.click()
            print(f"{Fore.CYAN}{EMOJI['WAIT']} 等待注册响应...{Style.RESET_ALL}")
            
            # 循环检查提示信息
            max_attempts = 10
            for i in range(max_attempts):
                has_error, tips = check_page_tips(page, "注册")
                if has_error:
                    return False
                elif any(tip['type'] == 'success' for tip in tips):
                    break
                elif i < max_attempts - 1:  # 如果不是最后一次尝试
            ...
            if "login" in current_url:
                print(f"{Fore.GREEN}{EMOJI['SUCCESS']} 注册成功，已跳转到登录页面{Style.RESET_ALL}")
            else:
                print(f"{Fore.YELLOW}{EMOJI['WARN']} 未检测到跳转到登录页面，请确认注册状态{Style.RESET_ALL}")
                input(f"{Fore.CYAN}请确认是否继续(按回车继续)...{Style.RESET_ALL}")
        ...
        if login_button:
            login_button.click()
            print(f"{Fore.CYAN}{EMOJI['WAIT']} 等待登录响应...{Style.RESET_ALL}")
            
            # 循环检查登录提示信息
            max_attempts = 10
            for i in range(max_attempts):
                has_error, tips = check_page_tips(page, "登录")
                if has_error:
                    return False
                elif any(tip['type'] == 'success' for tip in tips):
                    break
        ...
        try:
            ...
            if login_button:
                ...
                for _ in range(max_wait):
                    current_url = page.url
                    if "mailList" in current_url:  # 如果URL包含mailList，说明登录成功
                        success = True
                        break
                    time.sleep(1)
                
                if not success:
                    print(f"{Fore.RED}{EMOJI['ERROR']} 登录超时，请检查是否登录成功{Style.RESET_ALL}")
                    input(f"{Fore.CYAN}请确认是否继续(按回车继续)...{Style.RESET_ALL}")
