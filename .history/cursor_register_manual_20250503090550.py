...
class CursorRegistration:
    ...
    def setup_email(self):
        ...
        try:
            ...
            page.get("https://www.2925.com/")
            time.sleep(3)  # 等待页面加载
            
            # 点击注册按钮
            register_button = page.ele("//a[contains(text(), '注册')]")
            if register_button:
                register_button.click()
                time.sleep(2)
            
            # 生成随机邮箱名
            random_number = random.randint(7, 1000)
            email_name = f"chnanan72{random_number}"
            
            # 输入邮箱名
            email_input = page.ele("//input[@name='username']")
            if email_input:
            ...
            # 输入密码
            password = "Aa123456"
            password_input = page.ele("//input[@name='password']")
            if password_input:
                password_input.send_keys(password)
                time.sleep(1)
            
            # 确认密码
            confirm_password_input = page.ele("//input[@name='confirm_password']")
            ...
            print(f"{Fore.GREEN}{EMOJI['SUCCESS']} 注册成功！{Style.RESET_ALL}")
