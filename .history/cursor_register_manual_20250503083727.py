    ...
    def get_verification_code(self):
        ...
        try:
            ...
            for attempt in range(max_attempts):
                ...
                try:
                    # 查找所有时间标签
                    time_elements = verification_page.eles("css:.date-time-text")
                    latest_time = None
                    latest_mail_item = None
                    ...
                    for time_element in time_elements:
                        ...
                        if "今天" in time_text:
                            # 获取父级的邮件内容div
                            mail_item = time_element.parent.parent
                            # 在邮件内容中查找验证邮件标题
                            title = mail_item.ele("css:.mail-content-title")
                            if title and "Verify your email address" in title.text:
                                latest_time = time_text
                                latest_mail_item = mail_item
                                break
                    
                    # 如果找到验证邮件，点击进入
                    if latest_mail_item:
                        print(f"{Fore.GREEN}{EMOJI['SUCCESS']} 找到验证邮件，发送时间: {latest_time}{Style.RESET_ALL}")
                        # 点击邮件标题
                        title_element = latest_mail_item.ele("css:.mail-content-title")
                        if title_element:
                            title_element.click()
                            time.sleep(3)
