...
class CursorRegistration:
    ...
    def handle_2925_email_setup(self):
        ...
        # 读取已保存的账号信息
        email, email_password = read_2925_account()
        
        if email and email_password:
            print(f"\n{Fore.CYAN}{'='*50}{Style.RESET_ALL}")
            print(f"{Fore.CYAN}{EMOJI['INFO']} 检测到已保存的2925邮箱账号：{Style.RESET_ALL}")
            print(f"{Fore.GREEN}邮箱: {email}@2925.com{Style.RESET_ALL}")
            print(f"{Fore.CYAN}{'='*50}{Style.RESET_ALL}")
            
            while True:
                choice = input(f"\n{Fore.CYAN}{EMOJI['INFO']} 是否使用此账号进行Cursor注册？(Y/n/q退出): {Style.RESET_ALL}").lower()
                if choice == 'q':
                    print(f"\n{Fore.YELLOW}{EMOJI['INFO']} 操作已取消{Style.RESET_ALL}")
                    return False
                if choice == '' or choice == 'y':
                    print(f"\n{Fore.GREEN}{EMOJI['SUCCESS']} 将使用已保存的2925邮箱账号{Style.RESET_ALL}")
                    # 设置原始邮箱地址
                    ...
                    break
                print(f"{Fore.RED}{EMOJI['ERROR']} 请输入 Y(默认) 或 n 或 q{Style.RESET_ALL}")
        
        # 如果没有账号信息或选择不使用，让用户选择操作
        print(f"\n{Fore.CYAN}{'='*50}{Style.RESET_ALL}")
        print(f"{Fore.CYAN}{EMOJI['INFO']} 请选择操作：{Style.RESET_ALL}")
        print(f"{Fore.GREEN}1. 注册新的2925邮箱 (第18项功能){Style.RESET_ALL}")
        print(f"{Fore.GREEN}2. 登录已有2925邮箱 (第19项功能){Style.RESET_ALL}")
        print(f"{Fore.GREEN}q. 退出操作{Style.RESET_ALL}")
        print(f"{Fore.CYAN}{'='*50}{Style.RESET_ALL}")
        
        while True:
            choice = input(f"\n{Fore.CYAN}请输入选项 (1/2/q): {Style.RESET_ALL}").lower()
            if choice == 'q':
                print(f"\n{Fore.YELLOW}{EMOJI['INFO']} 操作已取消{Style.RESET_ALL}")
                return False
            if choice == "1":
                if setup_email(self.translator):
                    # 读取新注册的账号信息
                    new_email, email_password = read_2925_account()
                    if new_email:
                        # 设置原始邮箱地址
                ...
                if custom_2925_email(self.translator):
                    ...
                    if new_email:
                        ...
                        return True
                return False
            print(f"{Fore.RED}{EMOJI['ERROR']} 请输入 1 或 2 或 q{Style.RESET_ALL}")
